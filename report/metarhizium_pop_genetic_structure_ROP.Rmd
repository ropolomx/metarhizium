...
the analyses of both _M. robertsii_ and _M. brunnei_, for both the comparisons of agricultural vs. natural habitats and glaciated vs. 
non-glaciated locations. STRUCTURE is a Bayesian clustering method that finds the most likely number of population 
clusters (K) based on allelic frequency that maximizes.com Hardy-Weinberg equilibrium and minimize linkage disequilibrium. 
...
Optimal alignments for the multiple replicate cluster analyses were searched with CLUMPP [@jakobsson_clumpp_2007] to ensure cluster labels match across runs,
with LargeKGreedy method with the G' pairwise matrix similarity statistic, random input orders and 100 repeats. 
Cluster data processed with CLUMPP was plotted with the ggplot2 package of R.
...
The STRUCTURE analysis revealed that for 
_M.robertsii_ the true value of K was 3 clusters and for _M. brunneum_ K=2, when considering glaciated vs. non-glaciated locations
as prior locations. In the case of _M. robertsii_, the genetic clusters included an admixed individual (sample #347, 
Osoyoos, BC, Montane Cordilla South Ecozone, Agricultural)

### Agricultural vs. natural sites

The true value of K was 3 for _M. robertsii_ and 2 for _M. brunneum_ when considering agricultural vs. 
